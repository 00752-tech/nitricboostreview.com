...
export default function VitalityCalculator() {
  ...
  const getPersonalizedRecommendations = (
    ...
    if (scores.diet < 4) {
      specificRecs.push(
        "Prioritize increasing your intake of leafy greens, which are rich in nitrates that support nitric oxide production"
      );
    } else if (scores.diet < 7) {
      specificRecs.push(
        "Consider adding more nitrate-rich foods like beets, arugula, and spinach to your diet to naturally boost nitric oxide levels"
      );
    }

    if (scores.exercise < 4) {
      specificRecs.push(
        "Start with short, 10-minute walks twice daily and gradually increase duration and intensity"
      );
    } else if (scores.exercise < 7) {
      specificRecs.push(
        "Incorporate resistance training 2-3 times a week to improve overall cardiovascular health and nitric oxide production"
      );
    }

    if (scores.stress > 7) {
      specificRecs.push(
        "Practice the 4-7-8 breathing technique daily: inhale for 4 seconds, hold for 7, exhale for 8"
      );
    } else if (scores.stress > 5) {
      specificRecs.push(
        "Try a 10-minute guided meditation before bed to reduce stress and improve sleep quality"
      );
    }

    if (scores.sleep < 4) {
      specificRecs.push(
        "Create a sleep-conducive environment: use blackout curtains, white noise, and keep the room cool"
      );
    } else if (scores.sleep < 7) {
      specificRecs.push(
        "Limit caffeine intake after 2 PM and establish a relaxing pre-sleep routine"
      );
    }
  ...
  const calculateScore = () => {
    ...
    const result: VitalityResult = {
      ...
      ...(finalScore < 50 && {
        warning:
          "Given your current vitality score, we strongly recommend consulting with a healthcare professional for personalized advice before starting any new supplement or exercise regimen.",
      }),
    };
  ...
  const getResultMessage = (score: number, category: string): string => {
    ...
    return (
      messages[category] ||
      `Your vitality score is ${score}/100. There's always room for improvement in health and wellness.`
    );
  };
  ...
  return (
    <div className="bg-gray-900 p-8 rounded-xl shadow-lg max-w-3xl mx-auto">
      <h3 className="text-3xl font-bold mb-6 text-white">
        Personalized Vitality Assessment
      </h3>
      <p className="text-gray-300 mb-8">
        Complete this assessment to receive tailored recommendations for
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-400">
            Consider your daily intake of whole foods, vegetables, and lean
            proteins
          </p>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-sm text-gray-400">
            Consider both duration and quality of sleep
          </p>
        </div>
        ...
        {result && (
          <div className="mt-8 p-6 bg-gray-800/50 rounded-lg">
            ...
            <p className={`text-lg mb-6 ${getCategoryColor(result.category)}`}>
              {result.message}
            </p>
